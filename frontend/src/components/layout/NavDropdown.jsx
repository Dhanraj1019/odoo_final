...
export default function NavDropdown({ label, icon: Icon, items = [], basePath = "" }) {
  ...
  return (
    <div className="relative inline-block text-left" ref={dropdownRef}>
      ...
      {isOpen && (
        <div className="absolute left-0 mt-1.5 w-56 rounded-xl bg-white shadow-lg shadow-slate-900/10 border border-slate-200/80 py-1.5 z-50 animate-in fade-in zoom-in-95 duration-100">
          ...
          {items.map((item) => {
            ...
            return (
              ...
              >
                {ItemIcon && (
                  <ItemIcon
                    className={`w-4 h-4 ${isItemActive ? "text-indigo-600" : "text-slate-400"}`}
                  />
                )}
                <div>
